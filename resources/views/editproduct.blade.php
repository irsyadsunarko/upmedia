        ...
        <form action="{{ route('updateproduct') }}" method="POST" enctype="multipart/form-data">
            @csrf
                <div class=" bg-white rounded-lg shadow-lg py-3 px-5 relative mb-3">
                    ...
                    <input class="w-full font-bold text-lg focus:outline-none" type="text" value="{{$product->product_name}}" id="product_name" name="product_name" placeholder="Judul Product" required><br>
                </div>
                <div class=" bg-white rounded-lg shadow-lg py-3 px-5 relative mb-3">
                    <label for="product_picture" class="text-sm font-bold">Foto Product <span class="text-red-700">*</span></label><br>
                    <input id="product_picture" class="text-sm border border-black w-full rounded-lg py-1 px-2" type="file" name="product_picture" accept="image/*"><br>
                </div>                
                <div class=" bg-white rounded-lg shadow-lg py-3 px-5 relative mb-3">
                    <label for="product_description" class="text-sm font-bold">Deskripsi Product <span class="text-red-700">*</span></label><br>
                    <textarea rows="5" id="product_description" class="text-sm border border-black w-full rounded-lg py-1 px-2" name="product_description" required>{{$product->product_description}}</textarea><br>
                </div>      
                <div class=" bg-white rounded-lg shadow-lg py-3 px-5 relative mb-3">
                    ...
                    <select id="product_categories" class="text-sm border border-black w-full rounded-lg py-1 px-2" name="product_categories" required>
                        @foreach($categories as $category)                            
                            <option value="{{$category->id}}" @if($product->product_categories == $category->id) selected @endif>{{$category->categories_name}}</option>    
                        @endforeach
                    </select><br>
                </div>
            
            <input type="text" id="product_id" name="product_id" class="hidden" value="{{$product->id}}" readonly><br>
            <input type="text" id="created_by" name="created_by" class="hidden" value="{{$user->email}}" readonly><br>
            <input class="w-full py-3 bg-red-700 text-white rounded-lg cursor-pointer hover:bg-red-600" type="submit" value="Edit Product">
        </form>
    </div>
</section>
